...
class WindowController:
    def __init__(self, max_ips="auto"):
        ...
        try:
            ...
            self._reconnect_lock = threading.Lock()   # serialize reconnects across worker + main threads
            self.max_ips = max_ips
            # Capture below native res: the detection model only uses 640px, so a lighter
            # stream stabilises flaky emulator encoders (LDPlayer freezes/native decode
            # crashes) without hurting detection. max_width=0 restores full 1080p.
            _gc = load_toml_as_dict("cfg/general_config.toml")
            self.scrcpy_max_width = int(_gc.get("scrcpy_max_width", 1280))
            self.scrcpy_bitrate = int(_gc.get("scrcpy_bitrate", 4000000))
            self.scrcpy_client = scrcpy.Client(device=self.device, max_width=self.scrcpy_max_width, bitrate=self.scrcpy_bitrate) if self.max_ips == "auto" else scrcpy.Client(device=self.device, max_width=self.scrcpy_max_width, bitrate=self.scrcpy_bitrate, max_fps=self.max_ips)
        ...
        self.PID_JOYSTICK = 1
        self.PID_ATTACK = 2
        self.aim_drag_radius = load_toml_as_dict("cfg/bot_config.toml").get("aim_drag_radius", 140)  # design px; how far to drag a button to aim it
        
        self.action_queue = queue.Queue()
        self.input_worker = threading.Thread(target=self._input_worker_loop, daemon=True, name="pyla-input-worker")
    ...
    def _input_worker_loop(self):
        while True:
            ...
            try:
                if action == 'click':
                    self._click_sync(*args, **kwargs)
                elif action == 'press':
                    self._press_sync(*args, **kwargs)
                elif action == 'swipe':
                    self._swipe_sync(*args, **kwargs)
                elif action == 'aim_swipe':
                    self._aim_swipe_sync(*args, **kwargs)
    ...
    def reconnect_scrcpy(self, max_retries=3):
        # Only one reconnect at a time — the input worker thread and the main loop can
        # both trigger this, and running two concurrently stomps self.scrcpy_client.
        if not self._reconnect_lock.acquire(blocking=False):
            return False
        try:
            return self._reconnect_scrcpy_impl(max_retries)
        finally:
            self._reconnect_lock.release()

    def _reconnect_scrcpy_impl(self, max_retries=3):
    ...
    def _click_sync(self, x: int, y: int, delay=0.02, already_include_ratio=True, touch_up=True, touch_down=True):
    ...
    def click(self, x: int, y: int, delay=0.02, already_include_ratio=True, touch_up=True, touch_down=True, blocking=False):
        if blocking:
            self._click_sync(x, y, delay, already_include_ratio, touch_up, touch_down)
        else:
            if self.action_queue.qsize() <= 1:
                self.action_queue.put(('click', (x, y), {'delay': delay, 'already_include_ratio': already_include_ratio, 'touch_up': touch_up, 'touch_down': touch_down}))

    def _press_sync(self, key, delay=0.02, touch_up=True, touch_down=True):
        if key not in press_coords_dict:
            return
        x, y = press_coords_dict[key]
        target_x = x * self.width_ratio
        target_y = y * self.height_ratio
        self._click_sync(target_x, target_y, delay, touch_up=touch_up, touch_down=touch_down)

    def press(self, key, delay=0.02, touch_up=True, touch_down=True, blocking=False):
        if blocking:
            self._press_sync(key, delay, touch_up, touch_down)
        else:
            if self.action_queue.qsize() <= 1:
    ...
    def _aim_swipe_sync(self, key, dir_x, dir_y, duration=0.06, distance_ratio=1.0):
        if key not in press_coords_dict:
            return
        if self.width_ratio is None or self.scale_factor is None:
            self._press_sync(key)
            return
        bx, by = press_coords_dict[key]
        center_x = bx * self.width_ratio
        center_y = by * self.height_ratio
    ...
    def aim_swipe(self, key, dir_x, dir_y, duration=0.06, blocking=False, distance_ratio=1.0):
        if blocking:
            self._aim_swipe_sync(key, dir_x, dir_y, duration, distance_ratio)
        else:
            if self.action_queue.qsize() <= 1:
                self.action_queue.put(('aim_swipe', (key, dir_x, dir_y), {'duration': duration, 'distance_ratio': distance_ratio}))
