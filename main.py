...
def pyla_main(discord_bot, queue_data, stop_event=None, runtime_control=None):
    class Main:
        ...
        def handle_detected_state(self, state):
            ...
            print(f"State: {state}")
            if state == "lobby" and not getattr(self, "picked_first_brawler", False):
                print("Skipping lobby play action because first brawler is not picked yet.")
                return
            frame_data = None
        ...
        def main(self):
            ...
            while True:
                stoppable_states = [
                    "lobby", "shop", "brawler_selection", "popup", "trophy_reward", 
                    "prestige_milestone", "nano_noodles", "match_making",
                    "star_drop_regular", "star_drop_angelic", "star_drop_demonic", "star_drop_starr_nova"
                ]
                
                current_state = self.get_latest_state()
                if current_state in stoppable_states:
                    if self.should_stop():
                ...
                if not self.picked_first_brawler and current_state == "lobby":
                    if self.Stage_manager.brawlers_pick_data[0]['automatically_pick']:
                        ...
                        if self.runtime_control:
                            self.runtime_control.mark_running()
                        target_trophies = self.Stage_manager.brawlers_pick_data[0].get("push_until", 0)
                        select_brawler_res = self.lobby_automator.select_brawler(next_brawler_name, self.get_latest_state, runtime_control=self.runtime_control, target_trophies=target_trophies)

                        if isinstance(select_brawler_res, tuple):
                            status, act_brawler, act_trophies = select_brawler_res
                            if status == "completed":
                                print("Auto mode completed. Stopping.")
                                ...
                                return
                            if status == "success":
                                self.Stage_manager.brawlers_pick_data[0]["brawler"] = act_brawler
                                self.Stage_manager.brawlers_pick_data[0]["trophies"] = act_trophies if act_trophies is not None else self.Stage_manager.brawlers_pick_data[0]["trophies"]
                        ...
                        while status in ["failed", "error"]:
                            print(f"Automatic brawler selection failed with status: {status}")
